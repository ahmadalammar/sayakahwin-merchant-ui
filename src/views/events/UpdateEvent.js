...
const UpdateEvent = () => {
    ...
    const fetchEventData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          lang: data.lang || 'en',
          hero_main_date: data.hero_main_date ? String(data.hero_main_date).slice(0, 10) : '',
          hero_main_time_from: data.hero_main_time_from
            ? String(data.hero_main_time_from).slice(0, 5)
            : '',
          hero_main_location: data.hero_main_location || '',
        })
        setSchedules(
        ...
        const addPhoneRsvp = !(
          data.add_phonenumber === false ||
          data.add_phonenumber === 0 ||
          data.add_phonenumber === '0'
        )
        setAddPhonenumber(addPhoneRsvp)
        setCompulsoryPhonenumber(
          addPhoneRsvp &&
            !!data.compulsory_phonenumber &&
            data.compulsory_phonenumber !== 0 &&
            data.compulsory_phonenumber !== '0',
        )
        setAllowCheckin(data.allow_checkin || false)
        setIsReminderEnabled(!!data.is_reminder_enabled)
        setIsSeatingEnabled(!!data.is_seating_enabled)
        setIsGroupingFeatureEnabled(!!data.is_grouping_feature_enabled)
        setIsMixedEvent(!!data.is_mixed_event)
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      eventData.append('hero_main_date', formData.hero_main_date || '')
      eventData.append('hero_main_time_from', formData.hero_main_time_from || '')
      eventData.append('hero_main_location', formData.hero_main_location || '')
